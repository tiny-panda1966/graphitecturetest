...
const Section2 = {
    ...
    selectType(type) {
        if (type === orderState.type) { this.render(); return; } // same type, just re-render
        const doIt = () => {
        ...
        const hasProduct = !!(orderState.productId || orderState.variant || orderState.sizeEntries.length);
        hasProduct ? RestoreManager.guardChange(type, doIt) : doIt();
    },
    ...
    selectProduct(productId) {
        if (productId === orderState.productId) return; // same product
        const _doSelect = (productId) => {
        ...
        const hasDownstream = !!(orderState.variant || orderState.sizeEntries.length);
        hasDownstream ? RestoreManager.guardChange(null, () => _doSelect(productId)) : _doSelect(productId);
    }
};
