...
const Section5 = {
    ...
    _renderFootballMatchDay() {
        ...
        this.body.innerHTML = `
            ...
            ${this._optionRow('backNumbers','Numbers',numberPrice + '/pos',false)}
            <div class="sub-options" id="backNumbersSub">
                <div class="sub-option-group">
                    <div class="sub-option-label">Number Positions</div>
                    <label class="checkbox-row"><input type="checkbox" data-numpos="topLeftShirt"> Numbers - Back of Shirts</label>
                    <label class="checkbox-row"><input type="checkbox" data-numpos="bottomRightShorts"> Numbers - Shorts</label>
                    <label class="checkbox-row"><input type="checkbox" data-numpos="backOfShirt"> Numbers - Training</label>
                </div>
        ...
        if (type === 'football-matchday') {
            ...
            if (c.backNumbers.enabled) {
                Object.entries(c.backNumbers.positions || {}).forEach(([pos, checked]) => {
                    if (checked) {
                        const cb = this.body.querySelector(`[data-numpos="${pos}"]`);
    ...
    _bindFootballMatchDaySwitches() {
        ...
        // Number position checkboxes
        this.body.querySelectorAll('[data-numpos]').forEach(cb => {
            cb.addEventListener('change', () => { c.backNumbers.positions[cb.dataset.numpos] = cb.checked; orderState.recalculate(); OrderSummaryUI.update(); });
        });
