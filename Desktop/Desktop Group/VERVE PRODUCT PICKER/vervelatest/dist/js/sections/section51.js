...
const Section51 = {
    ...
    _dedupeSocksByTitle(items) {
        ...
        (items || []).forEach(item => {
            ...
            const mergedRows = [...(existing.rows || []), ...(item.rows || [])];
            const mergedGallery = [...(existing.mediaGallery || []), ...(item.mediaGallery || [])];
            map.set(key, {
                ...existing,
                ...item,
                rows: mergedRows,
                mediaGallery: mergedGallery.filter((g, idx, arr) => {
                    const sig = (g?.fileName || g?.title || g?.displayUrl || g?.src || '') + '|' + (g?.displayName || '');
                    return idx === arr.findIndex(x => ((x?.fileName || x?.title || x?.displayUrl || x?.src || '') + '|' + (x?.displayName || '')) === sig);
                }),
                leadeImage: existing.leadeImage || item.leadeImage,
    ...
    _getSockGroupQty(priceGroup) {
        return (orderState.sizeEntries || [])
            .filter(se => se.priceGroup === priceGroup)
            .reduce((sum, se) => sum + (se.quantity || 0), 0);
    },

    _getResolvedSockPrice(sockItem, variantName) {
        const rows = sockItem?.rows || [];
        let total = 0;
        let hasRowPrice = false;
        ['adult', 'junior'].forEach(group => {
            const qty = this._getSockGroupQty(group);
            if (!qty) return;
            const row = rows.find(r => r.group === group) || rows[0];
            const tierPrice = row?.tiers ? Helpers.getTierPrice(row.tiers, qty) : null;
            if (typeof tierPrice === 'number' && !Number.isNaN(tierPrice)) {
                total += tierPrice * qty;
                hasRowPrice = true;
            }
        });
        if (hasRowPrice) return total;

        const variantPrice = sockItem?.price?.variants?.find(p => p.colourName === variantName)?.price;
        if (typeof variantPrice === 'number') return variantPrice * (orderState.getTotalQuantity?.() || 1);

        const basePrice = sockItem?.price?.basePrice;
        if (typeof basePrice === 'number') return basePrice * (orderState.getTotalQuantity?.() || 1);
    ...
    onSocksData(data) {
        this.socksData = this._dedupeSocksByTitle(data.socks || data.items || data || []);
    ...
    render() {
        const socks   = this._dedupeSocksByTitle(this.socksData);
    ...
    _renderVariantGallery(variants, selected, sockItem) {
        ...
        variants.forEach((v, i) => {
            const rawName = v.title || v.fileName || v.displayName || `Option ${i + 1}`;
            const displayName = this._parseName(rawName);
            const imageUrl    = v.displayUrl || v.src || v.imageUrl || v.url || '';
            const variantPrice = this._getResolvedSockPrice(sockItem, displayName);
            const priceLabel  = ''; // price removed from image label
        ...
        const confirmHtml = selected
            ? `<div class="sock-selection-confirm">
                   ✓ ${sockItem?.title || ''} — ${selected.displayName}
               </div>`
            ...
            card.addEventListener('click', () => {
                ...
                orderState.customisation.socks = {
                    ...
                    sockType: {
                        ...
                        rows:       sockItem.rows || [],
                        sku:        sockItem.sku || 'SOCKS',
                        groupSkus:  sockItem.groupSkus || null
                    },
                    sockVariant: null,
                    price:       this._getResolvedSockPrice(sockItem, null),
                    _chosen:     true
                };
