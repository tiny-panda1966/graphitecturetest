...
const Section1 = {
    ...
    /** Build icon HTML — DB image if available, fallback SVG */
    _iconHtml(collection) {
        const sel = this.selections[collection];
        if (sel && sel.image) {
            return `<img src="${sel.image}" alt="${collection}" style="width:100%;height:100%;object-fit:contain;">`;
        }
        // Fallback SVGs
    ...
    render() {
        ...
        this.body.querySelectorAll('.selector-btn').forEach(btn => {
            btn.addEventListener('click', () => this.select(btn.dataset.collection));
        });
    },
