function TimesheetView({ timesheet, setTimesheet, addHistory, projectId, items }) {
  ...
  // ── Material totals from line items ──
  ...
  var totalMaterialCost = materialItems.reduce(function(a, m) { return a + m.cost; }, 0);
  var totalSqm = materialItems.reduce(function(a, m) { return a + m.sqm; }, 0);

  // ── Combined ──
  var totalProjectCost = totalLabourCost + totalMaterialCost;
  ...
  return (
    <div className="page-container">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
        <div><div style={s.pageTitle}>Timesheet & Costs</div><div style={s.pageSub}>Full project cost breakdown — materials + labour</div></div>
      ...
      <div className="grid-4" style={{ marginBottom: 20 }}>
        <StatCard value={fmt(totalMaterialCost)} label="Material Cost" />
        <StatCard value={fmt(totalLabourCost)} label="Labour Cost" />
        <StatCard value={fmt(totalProjectCost)} label="Total Project Cost" />
      ...
      <div style={Object.assign({}, s.card, { marginBottom: 20 })}>
        <div style={s.cardHead}>
          <span style={s.cardTitle}>Materials</span>
          <span style={{ fontSize: 11, color: "#888" }}>{materialItems.length} items · {totalSqm.toFixed(2)} m²</span>
        </div>
        <div className="table-wrap">
          <table style={Object.assign({}, s.table, { tableLayout: "fixed" })}>
            ...
            <thead><tr>
              {["Line Item", "Process", "Qty", "Sq m", "Rate (£/m²)", "Material Cost"].map(function(h) {
            ...
            <tbody>
              ...
              {materialItems.length === 0 && React.createElement("tr", null,
                React.createElement("td", { colSpan: 6, style: Object.assign({}, s.td, { textAlign: "center", color: "#999", padding: 20 }) }, "No line items with material costs")
              )}
            </tbody>
            {materialItems.length > 0 && React.createElement("tfoot", null,
              React.createElement("tr", { style: { borderTop: "2px solid #111" } },
                React.createElement("td", { colSpan: 3, style: Object.assign({}, s.td, { fontWeight: 500 }) }, "MATERIALS TOTAL"),
                ...
                React.createElement("td", { style: Object.assign({}, s.td, { textAlign: "right", fontWeight: 500 }) }, fmt(totalMaterialCost))
              )
            )}
          </table>
        </div>
      ...
      <div style={Object.assign({}, s.card, { marginTop: 20 })}>
        ...
        <div style={s.cardBody}>
          {[["Materials", fmt(totalMaterialCost)], ["Labour", fmt(totalLabourCost)], ["Total Project Cost", fmt(totalProjectCost)]].map(function(pair, idx) {
            var isTotal = idx === 2;
            return React.createElement("div", { key: idx, style: { display: "flex", justifyContent: "space-between", padding: isTotal ? "12px 0 0" : "8px 0", borderTop: isTotal ? "2px solid #111" : idx > 0 ? "1px solid #f5f5f5" : "none", fontSize: isTotal ? 16 : 13, fontWeight: isTotal ? 500 : 400 } },
              React.createElement("span", { style: { color: isTotal ? "#111" : "#666" } }, pair[0]),
