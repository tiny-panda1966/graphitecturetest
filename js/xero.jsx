function XeroExport({ items, timesheet, info, projectId, userEmail }) {
  ...
  // ── Pricing state ──
  var [defaultMargin, setDefaultMargin] = useState(50);
  var [lineMargins, setLineMargins] = useState({}); // { itemId: { margin: 50, salePrice: null, overridden: false } }

  // Calculate costs
  var totalMaterials = items.reduce(function(a, i) { return a + (i.cost || 0); }, 0);
  var totalLabour = timesheet.reduce(function(a, t) { return a + (t.cost || 0); }, 0);
  var totalCost = totalMaterials + totalLabour;

  // Get margin/sale for a line item
  var getLinePrice = function(item) {
    var materialCost = item.cost || 0;
    // Proportional labour allocation (by material cost share)
    var labourShare = totalMaterials > 0 ? (materialCost / totalMaterials) * totalLabour : 0;
    var lineCost = materialCost + labourShare;

    var override = lineMargins[item.id];
    var margin = override && override.overridden ? override.margin : defaultMargin;

    // Sale price: cost / (1 - margin/100) for true margin
    var salePrice;
    if (override && override.salePrice !== null && override.salePrice !== undefined) {
      salePrice = override.salePrice;
      // Recalculate margin from sale price
      margin = lineCost > 0 ? parseFloat(((1 - lineCost / salePrice) * 100).toFixed(1)) : 0;
    } else {
      salePrice = margin >= 100 ? lineCost * 10 : parseFloat((lineCost / (1 - margin / 100)).toFixed(2));
    }

    var isOverridden = override && override.overridden;
    return { materialCost: materialCost, labourShare: labourShare, lineCost: lineCost, margin: margin, salePrice: salePrice, isOverridden: isOverridden };
  };
  ...
  var blendedMargin = totalSale > 0 ? parseFloat(((1 - totalCost / totalSale) * 100).toFixed(1)) : 0;

  // Set margin for a line item
  var setLineMargin = function(itemId, margin) {
    setLineMargins(function(prev) {
      var next = Object.assign({}, prev);
      next[itemId] = { margin: margin, salePrice: null, overridden: true };
      return next;
    });
  };

  // Set sale price for a line item (overrides margin)
  var setLineSalePrice = function(itemId, price) {
    setLineMargins(function(prev) {
      var next = Object.assign({}, prev);
      next[itemId] = { margin: (prev[itemId] || {}).margin || defaultMargin, salePrice: price, overridden: true };
      return next;
    });
  };

  // Apply default margin to all
  var applyDefaultToAll = function() {
    setLineMargins({});
  };
        ...
        if (DB.isLive()) {
          var invoiceData = {
            ...
            lineItems: JSON.stringify(pricingRows.map(function(r) {
              return { desc: r.item.desc, process: r.item.process, materialCost: r.materialCost, labourShare: r.labourShare, lineCost: r.lineCost, margin: r.margin, salePrice: r.salePrice };
            })),
            totalCost: totalCost, totalSale: totalSale,
            totalVat: parseFloat((totalSale * 0.2).toFixed(2)),
            totalInc: parseFloat((totalSale * 1.2).toFixed(2)),
            blendedMargin: blendedMargin, defaultMargin: defaultMargin,
  ...
  return (
    <div className="page-container">
      ...
      <div style={s.pageSub}>Set sale prices, review margins, then generate invoice</div>

      {/* ── PRICING TABLE ── */}
      <div style={Object.assign({}, s.card, { marginBottom: 20 })}>
        <div style={s.cardHead}>
          <span style={s.cardTitle}>Pricing & Margins</span>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 11, color: "#888" }}>Default margin:</span>
            <input type="number" min="0" max="99" value={defaultMargin}
              onChange={function(e) { setDefaultMargin(parseFloat(e.target.value) || 0); }}
              style={Object.assign({}, s.input, { width: 50, textAlign: "center", padding: "4px 6px", fontSize: 12 })} />
            <span style={{ fontSize: 11, color: "#888" }}>%</span>
            <button style={Object.assign({}, s.btn("secondary"), { fontSize: 10, padding: "4px 10px" })} onClick={applyDefaultToAll}>Apply to All</button>
          </div>
        </div>
        <div className="table-wrap">
          <table style={s.table}>
            <thead><tr>
              {["Line Item", "Material", "Labour", "Total Cost", "Margin %", "Sale Price", "Profit"].map(function(h) {
                return React.createElement("th", { key: h, style: s.th }, h);
              })}
            </tr></thead>
            <tbody>
              {pricingRows.map(function(row) {
                var profit = row.salePrice - row.lineCost;
                return React.createElement("tr", { key: row.item.id },
                  React.createElement("td", { style: Object.assign({}, s.td, { fontWeight: 500 }) },
                    React.createElement("div", null, row.item.desc),
                    React.createElement("div", { style: { fontSize: 10, color: "#999" } }, row.item.process),
                    row.isOverridden && React.createElement("span", { style: { fontSize: 8, padding: "1px 6px", borderRadius: 3, background: "#fff3e0", color: "#e65100" } }, "Custom")
                  ),
                  React.createElement("td", { style: Object.assign({}, s.td, { textAlign: "right", fontSize: 11, color: "#888" }) }, fmt(row.materialCost)),
                  React.createElement("td", { style: Object.assign({}, s.td, { textAlign: "right", fontSize: 11, color: "#888" }) }, fmt(row.labourShare)),
                  React.createElement("td", { style: Object.assign({}, s.td, { textAlign: "right", fontWeight: 500 }) }, fmt(row.lineCost)),
                  React.createElement("td", { style: Object.assign({}, s.td, { textAlign: "center" }) },
                    React.createElement("input", { type: "number", min: "0", max: "99", step: "1",
                      value: row.margin,
                      onChange: function(e) { setLineMargin(row.item.id, parseFloat(e.target.value) || 0); },
                      style: Object.assign({}, s.input, { width: 50, textAlign: "center", padding: "4px 6px", fontSize: 11 })
                    })
                  ),
                  React.createElement("td", { style: Object.assign({}, s.td, { textAlign: "right" }) },
                    React.createElement("input", { type: "number", min: "0", step: "1",
                      value: row.salePrice,
                      onChange: function(e) { setLineSalePrice(row.item.id, parseFloat(e.target.value) || 0); },
                      style: Object.assign({}, s.input, { width: 80, textAlign: "right", padding: "4px 6px", fontSize: 11, fontWeight: 500 })
                    })
                  ),
                  React.createElement("td", { style: Object.assign({}, s.td, { textAlign: "right", fontWeight: 500, color: profit >= 0 ? "#2e7d32" : "#c62828" }) }, fmt(profit))
                );
              })}
            </tbody>
            <tfoot>
              <tr style={{ borderTop: "2px solid #111" }}>
                <td style={Object.assign({}, s.td, { fontWeight: 500 })}>TOTALS</td>
                <td style={Object.assign({}, s.td, { textAlign: "right", fontWeight: 500 })}>{fmt(totalMaterials)}</td>
                <td style={Object.assign({}, s.td, { textAlign: "right", fontWeight: 500 })}>{fmt(totalLabour)}</td>
                <td style={Object.assign({}, s.td, { textAlign: "right", fontWeight: 500 })}>{fmt(totalCost)}</td>
                <td style={Object.assign({}, s.td, { textAlign: "center", fontWeight: 500 })}>{blendedMargin}%</td>
                <td style={Object.assign({}, s.td, { textAlign: "right", fontWeight: 500 })}>{fmt(totalSale)}</td>
                <td style={Object.assign({}, s.td, { textAlign: "right", fontWeight: 500, color: totalSale - totalCost >= 0 ? "#2e7d32" : "#c62828" })}>{fmt(totalSale - totalCost)}</td>
              </tr>
            </tfoot>
          </table>
        </div>
        ...
        <div style={s.card}>
          ...
          <div style={s.cardBody}>
            ...
            <div style={{ marginTop: 16, padding: "10px 14px", background: "#f5f5f5", borderRadius: 4, fontSize: 10, color: "#888" }}>
              Note: Invoice shows sale prices only. Costs ({fmt(totalCost)}) and margin ({blendedMargin}%) are internal.
            </div>
          </div>
        </div>
        <div>
